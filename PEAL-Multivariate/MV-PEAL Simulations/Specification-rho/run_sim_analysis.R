rm(list=ls())
# ==============================================================================
# File: analyze_sim_specs_rho_levels.R
# Usage: Rscript analyze_sim_specs_rho_levels.R
# ==============================================================================
...
if(length(files) == 0) {
  ...
  # --- NEW: Map to "Correlation Levels" ---
  # Calculate sum of absolute correlations to determine "strength"
  dat <- dat %>% 
    mutate(rho_sum = abs(True_Rho12) + abs(True_Rho13) + abs(True_Rho23))
  
  # 1. Identify unique scenarios and sort them by rho_sum
  unique_scens <- unique(dat$Scenario[order(dat$rho_sum)])
  
  # 2. Create a named vector mapping "rho_0.1..." -> "Level 1"
  level_labels <- seq_along(unique_scens)
  names(level_labels) <- unique_scens
  
  # 3. Create a clean function for ggplot to use later
  ...
  # 4. Set Factor levels explicitly so ggplot orders them correctly
...
if(exists("dat")) {
  ...
  theme_professional <- theme_bw(base_size = 14) + 
    theme(
      ...
      # No angle needed for short labels like "Level 1"
  ...
  p3 <- ggplot(beta_final, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
    ...
    labs(title=expression(bold("Relative Efficiency (Variance Ratio)")), 
         ...
         x="Correlation Strength Level (Low -> High)", y="Variance Ratio") + theme_professional
  ...
  plot_re <- function(data) {
    ggplot(data, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
      ...
      scale_color_manual(values=my_colors) +
      scale_x_discrete(labels = clean_scenario_labels) +
      labs(title="Relative Efficiency (Variance Ratio)", x="Correlation Strength Level (Low -> High)", y="Var Ratio") + 
      theme_professional
  }
