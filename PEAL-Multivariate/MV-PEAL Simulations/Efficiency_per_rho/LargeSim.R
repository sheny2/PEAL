...
# True parameter vectors for later comparison (not including corr, which is rho-specific)
...
## ------------------------------------------------
## 5. One replicate wrapper (takes Sigma_e)
## ------------------------------------------------
...
## ------------------------------------------------
## 7. BIG DRIVER: run whole pipeline for one (rho12,rho13,rho23)
## ------------------------------------------------
run_sim_for_rhos <- function(rho12, rho13, rho23,
                             N = 100,
                             label = NULL) {
  if (is.null(label)) {
    label <- paste0("rho_",
  ...
  # Build correlation matrix and Sigma_e for this setting
  ...
  # Seed base can depend on label if you want, but constant is fine per rho-setting
  registerDoRNG(8848)
  ...
  results <- foreach(
    ...
    # Sigma_e is captured from the enclosing environment
    run_one_rep(sample(1:1e6, 1), Sigma_e)
  }
...
## ------------------------------------------------
## 8. Rho-grid and SLURM array dispatch
## ------------------------------------------------
...
names(rho_grid) <- sapply(rho_grid, function(r) {
...
if (is.na(task_id) || task_id < 1 || task_id > length(rho_grid)) {
  stop("Invalid SLURM_ARRAY_TASK_ID: ", task_id,
       ". It must be between 1 and ", length(rho_grid), ".")
}

r_vec  <- rho_grid[[task_id]]
label  <- names(rho_grid)[task_id]

cat("### SLURM_ARRAY_TASK_ID =", task_id, "###\n")
cat("### Running rho setting:", label, "###\n")

# Choose number of sims per rho-setting
N_sim <- 500

out_this <- run_sim_for_rhos(
  rho12 = r_vec[1],
  rho13 = r_vec[2],
  rho23 = r_vec[3],
  N     = N_sim,
