...
gamma_0_fixed <- c(-3, -2, -1) # Increasing intercept 
gamma_1 <- 0.5
gamma_2 <- 0.4
...
final_batch_df <- foreach(global_sim_id = start_idx:end_idx,
                          ...
                          .packages = c("data.table", "dplyr", "MASS", "Matrix", "mice")) %dopar% {
                            ...
                            N_pat <- sum(m_hosp)
                            n_visits <- sample(5:25, N_pat, replace=TRUE)
...
# ----------------------------------------------------------------
                            # REPLACEMENT: SEQUENTIAL MISSINGNESS (R DEPENDENCY)
                            # ----------------------------------------------------------------
                            dat_miss <- dat_full
                            ...
                            # -------------------------------------------------------
                            pmiss_1 <- plogis(gamma_0_fixed[1] + lp_base)
                            ...
                            pmiss_2 <- plogis(gamma_0_fixed[2] + lp_base + theta * is_na_1)
                            ...
                            pmiss_3 <- plogis(gamma_0_fixed[3] + lp_base - theta * (is_na_1 + is_na_2))
