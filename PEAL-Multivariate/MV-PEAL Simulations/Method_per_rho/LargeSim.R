...
##    (takes Sigma_e and fixed p1_base, p2_base, p3_base)
## ------------------------------------------------
...
## ------------------------------------------------
## 9. BIG DRIVER: run whole pipeline for one rho setting
## ------------------------------------------------

run_sim_for_rho <- function(rho12, rho13, rho23,
                            p1_base, p2_base, p3_base,
                            N = 200,
                            label = NULL) {
  if (is.null(label)) {
    label <- paste0("rho_",
                    paste(sprintf("%0.2f", c(rho12, rho13, rho23)),
                          collapse = "_"))
  }
  ...
  # Scenario-specific correlation matrix and Sigma_e
  ...
  # Save raw results list for this rho setting
...
## ------------------------------------------------
## 10. rho grid and SLURM array dispatch
## ------------------------------------------------
...
if (is.na(task_id) || task_id < 1 || task_id > nrow(rho_meta)) {
  stop("Invalid SLURM_ARRAY_TASK_ID: ", task_id,
       ". It must be between 1 and ", nrow(rho_meta), ".")
}

this_rho <- rho_meta[task_id, ]

label <- this_rho$setting
rho12 <- this_rho$rho12
rho13 <- this_rho$rho13
rho23 <- this_rho$rho23

cat("### SLURM_ARRAY_TASK_ID =", task_id, "###\n")
cat("### Running rho setting:", label, "\n")

# Choose number of sims per setting
N_sim <- 500

out_this <- run_sim_for_rho(
  rho12   = rho12,
  rho13   = rho13,
  rho23   = rho23,
  p1_base = p1_base,
...
cat("Finished rho setting:", label, "\n")
