...
## ------------------------------------------------
## 1a. FIXED missingness (same for all scenarios)
## ------------------------------------------------
...
## ------------------------------------------------
## 1b. Fixed true residual correlation 
## ------------------------------------------------
...
corr_mat_true <- matrix(c(
  1,          rho12_true, rho13_true,
  rho12_true, 1,          rho23_true,
  rho13_true, rho23_true, 1
), nrow = py, byrow = TRUE)
...
##    (takes Sigma_e and fixed p1_base, p2_base, p3_base, and m_per_site)
## ------------------------------------------------

simulate_one <- function(seed, Sigma_e, p1_base, p2_base, p3_base, m_per_site) {
  set.seed(seed)
  
  # --- DGP ---
  # Same number of patients per site, controlled by m_per_site
  m_hosp <- rep(m_per_site, H)   # H sites, each with m_per_site patients
...
## ------------------------------------------------
## 6. Model fitting: OBS_UN (patterns), CC_UN, MICE_UN, FULL_UN
## ------------------------------------------------
...
## ------------------------------------------------
## 7. One replicate wrapper
## ------------------------------------------------

run_one_rep <- function(seed, Sigma_e, p1_base, p2_base, p3_base, m_per_site) {
  sim <- simulate_one(seed, Sigma_e, p1_base, p2_base, p3_base, m_per_site)
...
## ------------------------------------------------
## 9. BIG DRIVER: run whole pipeline for one sample-size setting
## ------------------------------------------------

run_sim_for_n <- function(m_per_site,
                          p1_base, p2_base, p3_base,
                          N = 200,
                          label = NULL) {
  if (is.null(label)) {
    label <- paste0("n_", m_per_site, "_per_site")
  }
  cat("=== Running simulation for", label, "===\n")
  cat("   Patients per site =", m_per_site, "\n")
  cat("   Total patients     =", H * m_per_site, "\n")
  cat("   Missingness: p1_base =", p1_base,
      "p2_base =", p2_base,
      "p3_base =", p3_base, "\n")
  
  # corr_mat_true, Sigma_e, true_corr_vec are global
  ...
  results <- foreach(
    ...
    run_one_rep(sample(1:1e6, 1), Sigma_e, p1_base, p2_base, p3_base, m_per_site)
  }
  
  stopCluster(cl)
  
  # Save raw results list for this sample-size setting
  ...
  invisible(list(
    results      = results,  # list of per-sim fits
...
## ------------------------------------------------
## 10. Sample-size grid and SLURM array dispatch
## ------------------------------------------------

n_grid <- c(20, 40, 60, 80, 100, 120, 140, 160)

# Get SLURM array index (default to 1 if not running under SLURM)
task_id <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID", "1"))

if (is.na(task_id) || task_id < 1 || task_id > length(n_grid)) {
  stop("Invalid SLURM_ARRAY_TASK_ID: ", task_id,
       ". It must be between 1 and ", length(n_grid), ".")
}

m_per_site <- n_grid[task_id]

label <- paste0("n_", m_per_site, "_per_site")

cat("### SLURM_ARRAY_TASK_ID =", task_id, "###\n")
cat("### Running sample-size setting:", label, "\n")

# Choose number of sims per setting
N_sim <- 500

out_this <- run_sim_for_n(
  m_per_site = m_per_site,
  p1_base    = p1_base,
  p2_base    = p2_base,
  p3_base    = p3_base,
  N          = N_sim,
...
cat("Finished sample-size setting:", label, "\n")
