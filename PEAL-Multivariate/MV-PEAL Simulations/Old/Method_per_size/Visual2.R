...
## ------------------------------------------------
## 1. Sample-size grid metadata (to attach n info)
## ------------------------------------------------

n_grid   <- c(20, 40, 60, 80, 100, 120, 140, 160)
n_labels <- paste0("n_", n_grid, "_per_site")
...
## ------------------------------------------------
files <- list.files(pattern = "^AllOut_n_.*_per_site\\.rds$")
if (length(files) == 0L) stop("No AllOut_n_*_per_site.rds files found in working directory.")
...
## ------------------------------------------------
## 3. Loop over each n_per_site setting, compute summaries
## ------------------------------------------------
for (f in files) {
  cat("Processing", f, "...\n")
  obj <- readRDS(f)
  
  ## setting label, e.g. "AllOut_n_20_per_site.rds" -> "n_20_per_site"
  lab <- gsub("^AllOut_|\\.rds$", "", f)
  
  ## match to n_meta
  ng <- n_meta %>% filter(setting == lab)
  if (nrow(ng) != 1) stop("Cannot match setting label in n_meta for file: ", f)
  
  n_per_site <- ng$n_per_site
  N_total    <- ng$N_total
  ...
  beta_summary <- beta_merged %>%
    group_by(setting, n_per_site, N_total, Model, Parameter) %>%
    summarise(
...
## ------------------------------------------------
## 4. Visualizations (x-axis: n_per_site)
## ------------------------------------------------

## 4a. Betas: bias, RE-variance, coverage vs n_per_site
...
## |Bias(β)| vs sample size per site
p_beta_bias <- ggplot(
  beta_summary_all_df,
  aes(x = n_per_site, y = abs(bias), color = Model, group = Model)
) +
...
p_beta_var <- ggplot(
  beta_RE,
  aes(x = n_per_site, y = log(RE), color = Model, group = Model)
) +
...
p_beta_cov <- ggplot(
  beta_summary_all_df,
  aes(x = n_per_site, y = coverage, color = Model, group = Model)
) +
...
sigmaRE_RE <- sigmaRE_summary_all_df %>%
  group_by(setting, n_per_site, Parameter) %>%
  mutate(
    var_full = variance[Model == "FULL_UN"],
    RE       = variance / var_full
  ) %>%
...
corr_RE <- corr_offdiag %>%
  group_by(setting, n_per_site, Parameter_plot) %>%
  mutate(
    var_full = variance[Model == "FULL_UN"],
    RE       = variance / var_full
  ) %>%
...
p_corr_var <- ggplot(
  ...
  labs(
    ...
    title = expression("Relative efficiency of off-diagonal " * rho * " estimates")
  )
