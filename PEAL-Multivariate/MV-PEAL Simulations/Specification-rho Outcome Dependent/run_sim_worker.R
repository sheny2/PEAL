...
final_batch_df <- foreach(global_sim_id = start_idx:end_idx,
  ...
  if(min(eigen_vals) <= 1e-6) return(NULL) 
  ...
  # ----------------------------------------------------------------
  # --- APPLY MISSINGNESS (SEQUENTIAL DEPENDENCY) ---
  # ----------------------------------------------------------------
  dat_miss <- dat
  
  # FIXED missingness params (since we are varying rho, not gamma)
  fixed_gamma <- c(-2.2, -2.2, -2.2) 
  
  # Linear Predictor Base
  lp_base <- 0.2 * dat$X1 + 0.1 * dat$X2 
  
  # Interaction Strength
  theta <- 2
  
  # 1. Y1 Missingness: Depends only on X
  pmiss_1 <- plogis(fixed_gamma[1] + lp_base)
  is_na_1 <- rbinom(N_obs, 1, pmiss_1)
  
  # 2. Y2 Missingness: Depends on X AND R1
  # "When Y1 missing (R1=1), Y2 likely missing."
  pmiss_2 <- plogis(fixed_gamma[2] + lp_base + theta * is_na_1)
  is_na_2 <- rbinom(N_obs, 1, pmiss_2)
  
  # 3. Y3 Missingness: Depends on X AND R1, R2
  # "When Y1, Y2 missing (R=1), Y3 likely observed"
  pmiss_3 <- plogis(fixed_gamma[3] + lp_base - theta * (is_na_1 + is_na_2))
  is_na_3 <- rbinom(N_obs, 1, pmiss_3)
  
  # 4. Safety Check: Prevent rows where ALL Ys are missing
