rm(list=ls())
# ==============================================================================
# File: analyze_sim_pattern_synthesis.R
# Usage: Rscript analyze_sim_pattern_synthesis.R
# ==============================================================================
suppressPackageStartupMessages({
  ...
  library(gridExtra)
  library(stringr)
  library(grid) # For textGrob title
})
...
sigma_long <- dat %>% dplyr::select(Scenario_F, Model, all_of(sigma_cols)) %>%
...
# --- CUSTOM LOGIC FOR BETA OUTCOME LABELS ---
# Map Parameter index to Outcome Number
get_outcome_label <- function(param_name) {
  num <- as.numeric(gsub("Beta", "", param_name))
  # px=2, so 1-2 -> Outcome 1, 3-4 -> Outcome 2, 5-6 -> Outcome 3
...
# --- BETA (With Updated Facet Labels) ---
p_beta_bias <- ggplot(beta_final, aes(x=Scenario_F, y=Bias, color=Model, group=Model)) +
  geom_hline(yintercept=0, linetype="dashed", color="grey50") +
  geom_line() + geom_point(size=1) + 
  facet_wrap(~Facet_Label, nrow=1, labeller=label_parsed) + # Using label_parsed for math notation
...
p_beta_cov <- ggplot(beta_final, aes(x=Scenario_F, y=Coverage, color=Model, group=Model)) +
  ...
  scale_color_manual(values=my_colors) + coord_cartesian(ylim=c(0, 1.0)) +
...
p_beta_re <- ggplot(beta_final, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
  ...
  labs(title="Fixed-effects: Log Relative Variance", 
       color ="Outcome Missingness Method",
       x="Missingness Level", y="Log RE")
...
p_sig_re <- ggplot(sig_sub, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
  ...
  scale_color_manual(values=my_colors) + theme_bottom +
  scale_x_discrete(labels=clean_scenario) +
  labs(title="Variance: Log Relative Variance", x="Missingness Level", y="Log RE")
...
p_rho_re <- ggplot(rho_sub, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
  ...
  scale_color_manual(values=my_colors) + theme_bottom +
  scale_x_discrete(labels=clean_scenario) +
  labs(title="Correlation: Log Relative Variance", x="Missingness Level", y="Log RE")
...
# Remove legend from bottom plots for cleaner grid
...
# Layout
lay_mat <- rbind(c(1,1), c(2,2), c(3,3),  # Beta Section
                 c(4,5),                  # Sigma/Rho Bias Section
                 c(6,7),                  # Sigma/Rho RE Section
                 c(8,8))                  # Legend

# Save big plot
pdf("plots/Method-Missing.pdf", width=14, height=16)
grid.arrange(p_beta_bias, p_beta_cov, p_beta_re, 
             p_sig_bias, p_rho_bias, 
             p_sig_re, p_rho_re, 
             mylegend, 
             layout_matrix = lay_mat,
             heights = c(1, 1, 1.2, 1, 1.2, 0.2) # Adjust heights
             # top = textGrob("MV-PEAL Under Varying Missingness Levels", 
             #                gp=gpar(fontsize=18, fontface="bold")) #  title optional
             )
dev.off()

cat("Synthesis plot saved to 'plots/Method-Missing.pdf'\n")