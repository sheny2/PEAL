...
final_batch_df <- foreach(global_sim_id = start_idx:end_idx,
                          ...
                          .packages = c("data.table", "dplyr", "MASS", "Matrix", "mice")) %dopar% {
                            ...
                            # We use this to force the correlation structure requested.
                            theta <- 2
                            ...
                            # -------------------------------------------------------
                            # Logic: "When Y1 observed (R1=0), Y2 likely observed."
                            #        "When Y1 missing (R1=1), Y2 likely missing."
                            # Implementation: We ADD (theta * is_na_1).
                            # If is_na_1 = 1, the logit increases -> Prob(Miss) increases.
                            ...
                            # -------------------------------------------------------
                            # Logic: "When Y1, Y2 observed (R=0), Y3 likely missing" (Baseline behavior defined by Gamma)
                            #        "When Y1, Y2 missing (R=1), Y3 unlikely missing"
                            # Implementation: We SUBTRACT (theta * (is_na_1 + is_na_2)).
                            # If is_na_1=1 and is_na_2=1, the logit decreases by 2*theta.
                            # This drives the probability of missingness for Y3 close to 0.
                          ...
                          # If all are missing, reset Y1 observed
                            ...
                            # 3. Complete Case (CC)
                            # Filter to keep only rows where ALL Ys are observed
                            data_list[["CC"]] <- dat_miss[complete.cases(dat_miss[, Y_cols]), ]
                            
                            # 4. MICE (Imputation per site)
                            # We wrap this in tryCatch because imputation might fail on weird random draws
                            dat_mice <- tryCatch({
                              imp_parts <- list()
                              # Split by site to impute locally
                              ...
                              for(s_id in names(site_indices)) {
                                ...
                                # Quick check: if a variable is 100% missing in this site, MICE will fail/drop it.
                                # We attempt basic PMM. m=1 for speed in simulation.
                                # maxit=5 is usually sufficient for convergence in simple cases.
                                mice_out <- mice(sub_d, m=1, 
                                                  # method='pmm', 
