rm(list=ls())
# ==============================================================================
# File: analyze_sim_specs_synthesis.R
# Usage: Rscript analyze_sim_specs_synthesis.R
# ==============================================================================
...
# Define Factor Order: Unstructured is Reference
...
# Check if Unstructured exists to calculate RE (Updated capitalization here too)
...
# Color Scheme (Keys must match capitalized factor levels)
...
p_re <- ggplot(beta_final, aes(x=Scenario_F, y=RE, color=Model, group=Model)) +
  ...
  labs(title="Log Relative Variance (vs Unstructured)", 
       ...
       color = "Correlation Structure", # Updated Legend Title
       x="Missingness Level", y="Log Variance Ratio")
...
# Layout Matrix: Top=Bias, Bottom=RE, Very Bottom=Legend
lay_mat <- rbind(c(1,1),
                 c(2,2),
                 c(3,3))

if(!dir.exists("plots")) dir.create("plots")
pdf("plots/Specs-Comparison-Synthesis.pdf", width=14, height=10)
grid.arrange(p_bias, p_re, mylegend, 
             layout_matrix = lay_mat,
             heights = c(1, 1.2, 0.2))
dev.off()

cat("Synthesis plot saved to 'plots/Specs-Comparison-Synthesis.pdf'\n")