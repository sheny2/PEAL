suppressPackageStartupMessages({
  ...
  library(mice)    # for site-wise imputation
})
...
## Generic function: fit three corstr on any COMPLETE data using peal.fit.RI_mv
fit_three_models <- function(dat, prefix) {
  Y <- as.matrix(dat[, Y_cols])
  X <- as.matrix(dat[, X_cols])
  Z <- build_Z_list(dat, H)
  id.site <- dat$site
  ...
  # Construct list, then assign names
  out <- list(res_in, res_ex, res_un)
  names(out) <- paste0(prefix, c("_IN", "_EX", "_UN"))
  out
}


## Pattern-based methods on incomplete data
...
run_one_rep <- function(seed) {
  ...
  # FULL: complete data
  full_res <- fit_three_models(sim$full, "FULL")
  
  # Pattern-based on incomplete data
  patt_res <- fit_pattern_models(sim$miss)
  
  ## Complete-case on rearranged_data_with_missing ---
  dat_cc <- sim$miss[stats::complete.cases(sim$miss[, Y_cols, drop = FALSE]), ]
  if (nrow(dat_cc) > 0) {
    dat_cc$site <- factor(dat_cc$site, levels = 1:H)
    cc_res <- fit_three_models(dat_cc, "CC")
  } else {
    # fallback if complete cases vanish
    cc_res <- lapply(c("CC_IN", "CC_EX", "CC_UN"), function(nm) {
      list(
        b        = rep(NA_real_, px * py),
        sigmaRE  = rep(NA_real_, H + 1),
        sigma_e  = NA_real_,
    ...
    names(cc_res) <- c("CC_IN","CC_EX","CC_UN")
  }
  
  ## MICE per site on rearranged_data_with_missing ---
  sites <- unique(sim$miss$site)
  imp_list <- vector("list", length(sites))
  names(imp_list) <- as.character(sites)
  for (sid in sites) {
    sd <- sim$miss[sim$miss$site == sid, , drop = FALSE]
    imp <- mice::mice(sd, m = 1, printFlag = FALSE)
    imp_list[[as.character(sid)]] <- mice::complete(imp)
  }
  dat_mice <- do.call(rbind, imp_list)
  dat_mice$site <- factor(dat_mice$site, levels = 1:H)
  mice_res <- fit_three_models(dat_mice, "MICE")
  
  # Combine all 12 models
  c(full_res, patt_res, cc_res, mice_res)
}
...
saveRDS(results, file = "Simulation_All_Case_Results.rds")
results <- readRDS("Simulation_All_Case_Results.rds")
...
## ------------------------------------------------
## 9. Example plots: bias comparisons
## ------------------------------------------------
...
# Correlation bias: off-diagonals, you can choose models of interest
corr_long_offdiag <- corr_long %>%
  dplyr::mutate(i = as.integer(substr(Parameter, 5, 5)),
