## =========================
## Multivariate PEAL (MV-PEAL) RI only (tried a different optimization functions)
...
lmm.profile3_mv <- function(par,
  ...
  if (reml) {
    # s2 <- qterm / (NtotR - pxR)
    rB <- Matrix::rankMatrix(bterm1, tol = 1e-10)  # numeric rank
...
peal.fit.RI_mv <- function(Y, X, Z, id.site, weights = NULL,
  ...
  # bounds
  # --- bounds: theta >= 1e-5; rho in (-1/(R-1)+eps, 1-eps) ---
  lower <- rep(1e-5, length(mypar.init))
  upper <- rep( Inf,  length(mypar.init))
  if (corstr == "exchangeable" && estimate_rho) {
    rc <- .Rcorr_inv_and_logdet(R, rho = 0)  # for feasible interval
    lower[length(lower)] <- rc$lower + 1e-8
    upper[length(upper)] <- rc$upper - 1e-8
  }
  
  # --- optimizer: nlminb (box constraints supported) ---
  # fn should return *scalar* objective to minimize (i.e., -profile loglik)
  res <- nlminb(start   = mypar.init,
                objective = fn,
                lower   = lower,
                upper   = upper,
                control = list(
                  eval.max = 2000,   # total function eval cap
                  iter.max = 1500,   # iteration cap
                  rel.tol  = 1e-8,   # relative tolerance (if available)
                  x.tol    = 1e-10,  # parameter tolerance
                  abs.tol  = 1e-10   # objective tolerance
                ))
  
  # Optional: gentle boundary nudge if rho lands exactly on a bound
  if (corstr == "exchangeable" && estimate_rho) {
    rh_idx <- length(res$par)
    res$par[rh_idx] <- min(max(res$par[rh_idx], lower[rh_idx] + 1e-10), upper[rh_idx] - 1e-10)
  }
  
  # --- (approx) Hessian after convergence ---
  # nlminb does not return a Hessian; use optimHess for a numerical approximation.
  H <- tryCatch(optimHess(res$par, fn), error = function(e) NULL)
  res$hessian <- H
  
  # Diagnostics similar to before
  if (verbose) {
    ok <- is.null(res$convergence) || res$convergence == 0
    cat(ifelse(ok, "Convergence reached", "Non-convergence!"), "\n")
    if (!is.null(res$message)) cat("nlminb message:", res$message, "\n")
    if (!is.null(res$hessian)) {
      evals <- tryCatch(eigen(res$hessian, symmetric = TRUE, only.values = TRUE)$values, error = function(e) NA)
      cat("Hessian PD:", ifelse(all(is.finite(evals)) && all(evals > 0), "yes", "no"), "\n")
    }
  }
