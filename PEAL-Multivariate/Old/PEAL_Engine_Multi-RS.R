## =========================
## Multivariate PEAL (MV-PEAL) RS (one variable)
...
# Build Z as block-diagonal over sites: [site_RI | patient_RI | patient_RS]
build_Z_block <- function(df_site, x_col) {
  N_h <- nrow(df_site)

  # 1) site random intercept: one column of ones for this site's rows
  Z_site_RI <- matrix(1, nrow = N_h, ncol = 1)

  # 2) patient random intercepts: one column per (site, patient)
  #    model.matrix gives a sparse incidence matrix with columns per patient ID
  Z_pt_RI <- as.matrix(model.matrix(~ 0 + factor(patient), data = df_site))

  # 3) patient random slopes: multiply each row by the random-slope covariate
  x_vec <- df_site[[x_col]]
  Z_pt_RS <- Z_pt_RI * x_vec  # row-wise recycling does the right thing

  # Combine for this site
  cbind(Z_site_RI, Z_pt_RI, Z_pt_RS)
}
...
lmm.profile3_mv <- function(par,
  ...
  for (h in seq_along(id.site.uniq)) {
    ...
    theta_u  <- par[1]
    theta_vh <- par[1 + h]
    theta_vh_rs <- par[[1 + K + h]]
    # V0 <- diag(c(theta_u, rep(theta_vh, pzh - 1)), pzh)          # p_zh x p_zh (per outcome)
    V0 <- diag(c(theta_u, rep(theta_vh, (pzh - 1)/2), rep(theta_vh_rs, (pzh - 1)/2)), pzh)
...
# -------------------------
peal.fit.RS_mv <- function(Y, X, Z, id.site, weights = NULL,
  ...
  if (is.null(mypar.init)) {
    ...
    if (verbose) cat('Default mypar.init (theta_u + theta_v[h] + theta_vh_rs[h]) =', mypar.init, '\n')
  } else {
