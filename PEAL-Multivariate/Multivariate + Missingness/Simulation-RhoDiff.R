...
sigma_u_true      <- 0.5
sigma_v_hosp_true <- runif(H, min = 0.4, max = 0.7)   # fixed across sims
sigma_e_true      <- 0.3
...
run_one <- function(seed) {
  ...
  fit <- tryCatch(
    peal.em.fit.RI_mv(
      ...
      em_iter    = 1,       # can increase if you want more EM refinement
      reml       = TRUE,
      verbose    = FALSE
    ),
  ...
  # Random effects SDs: theta * s2 (length H+1: site + H hospitals)
...
ggplot(beta_long, aes(x = Parameter, y = Bias)) +
  geom_boxplot(outlier.alpha = 0.2, fill = "skyblue", alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Bias Distribution of Fixed Effects (β)") +
  ylab("Bias (Estimate − True)") +
  xlab("Parameter")


# Random effects SD bias
ggplot(sigmaRE_long, aes(x = Parameter, y = Bias)) +
  geom_boxplot(outlier.alpha = 0.2, fill = "lightgreen", alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Bias Distribution of Random Effect SDs") +
  ylab("Bias (Estimate − True)") +
  xlab("Parameter")


ggplot(sigmae_df, aes(x = "sigma_e", y = Bias)) +
  geom_boxplot(outlier.alpha = 0.3, fill = "orange", alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  theme_bw() +
  ggtitle("Bias Distribution of Residual SD (sigma_e)") +
  ylab("Bias (Estimate − True)") +
  xlab("")


ggplot(corr_long, aes(x = Parameter, y = Bias)) +
  geom_boxplot(outlier.alpha = 0.2, fill = "plum", alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Bias Distribution of Correlation Estimates") +
  ylab("Bias (Estimate − True)") +
