...
```{r}
sim_data <- list(
  full_data = rearranged_data,
  observed_data = rearranged_data_miss,
  parameters = list(
    beta = beta,
    sigma_u = sigma_u,
    sigma_v_hosp = sigma_v_hosp,
    sigma_e = sigma_e,
    rho_true_vec = rho_true_vec
  )
)
saveRDS(sim_data, file = "simulated_mv_data.rds")
...
```{r}
# --- 2. Fit Model (Unstructured) of full data ---
fit_mv <- peal.fit(
...
```{r}
# --- 2. Fit Model (Unstructured) --- results should agree with full data 
fit_mv <- peal.fit(
