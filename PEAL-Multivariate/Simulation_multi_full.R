...
# Variance parameters: sigma_u, sigma_v(1..H), sigma_e, rho12, rho13, rho23
# Total params = 1 (u) + H (v) + 1 (e) + 3 (rho)
...
results <- foreach(k = 1:N_sim, 
     ...
     # Extract unstructured correlations from the estimated matrix
     # fit_mv$Corr is the full R x R matrix
     C_est <- fit_mv$Corr
...
if (length(results) > 0) {
  ...
  # True sigma vector must match the row order: u, v_hosp, e, rho12, rho13, rho23
  ...
  p1 <- beta_df %>% 
    ...
    labs(title = "Bias in Beta (Unstructured Corr)", y = "Bias")
  ...
  p2 <- sigma_df %>% 
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title = "Bias in Variance/Corr Components (Unstructured)", y = "Bias")
