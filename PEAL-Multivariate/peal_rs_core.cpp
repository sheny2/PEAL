...
List compute_site_stats_rs_cpp(List ShPat, vec par, int R, int px, mat Corr_f) {
  ...
  for (int h = 0; h < K; ++h) {
    ...
    for (int k = 0; k < S_h.size(); ++k) {
      ...
      Sxx += kron(Ri_emb, as<mat>(S["ShX"])); Sxz += kron(Ri_emb, as<mat>(S["ShXZ"])); Szz += kron(Ri_emb, as<mat>(S["ShZ"]));
      mat ShXY_f = zeros(px, R); ShXY_f.cols(o) = as<mat>(S["ShXY"]) * Ri;
      mat ShZY_f = zeros(pzh, R); ShZY_f.cols(o) = as<mat>(S["ShZY"]) * Ri;
      sxy += vectorise(ShXY_f); szy += vectorise(ShZY_f); yty += trace(Ri * as<mat>(S["ShYY"]));
...
vec compute_gradient_rs_cpp(List ShPat, vec par, int R, int px, mat Corr_f, vec beta, double s2) {
  ...
  for (int h = 0; h < K; ++h) {
    ...
    for (int k = 0; k < S_h.size(); ++k) {
      List S = S_h[k]; uvec o = as<uvec>(S["idx_outcomes"]) - 1; 
      mat Ri = inv_sympd(Corr_f.submat(o, o) + eye(o.n_elem,o.n_elem)*1e-9); mat Ri_emb = zeros(R, R); Ri_emb.submat(o, o) = Ri;
      Sxz += kron(Ri_emb, as<mat>(S["ShXZ"])); Szz += kron(Ri_emb, as<mat>(S["ShZ"]));
      mat ShZY_f = zeros(pzh, R); ShZY_f.cols(o) = as<mat>(S["ShZY"]) * Ri; szy += vectorise(ShZY_f);
    }
