...
lmm.profile3 <- function(par, pooled = FALSE, reml = TRUE,
  ...
  for (h in seq_along(id.site.uniq)) {
    ...
    q_start <- 3 + (h - 1) * (1 + R)
    q_end <- q_start + R - 1
    q_hr <- par[q_start:q_end]

    if(pzh != length(c(u, rep(v_h, mh), rep(q_hr, mh)))){cat("V hsa dim problem")}
    V <- diag(c(u, rep(v_h, mh), rep(q_hr, mh)), pzh)
    ...
    # Optimized version:
    V_diag <- diag(V)  # Extract diagonal elements of V
    A <- Diagonal(pzh, 1) + ShZ %*% Diagonal(pzh, V_diag)
    logdet <- determinant(A, logarithm = TRUE)$modulus
    lpterm1 <- lpterm1 + as.numeric(logdet)
...
lmm.fit3 <- function(Y = NULL, X = NULL, Z = NULL, id.site = NULL, weights = NULL,
                     ...
                     mypar.init = NULL, hessian = FALSE, verbose = T, RScol = NULL) {
  if (pooled) {
    id.site.uniq <- unique(id.site)
  ...
  if (common.s2) {
    ...
    fn <- function(parameter) {
      return(-lmm.profile3(par = parameter, pooled = FALSE, reml, Y, X, Z, id.site, weights, RScol, ShXYZ)$lp)
    }
    ...
    mypar <- res$par
    res.profile <- lmm.profile3(par = mypar, pooled = FALSE, reml, Y, X, Z, id.site, weights, RScol, ShXYZ)
    s2 <- res.profile$s2
    V <- mypar * s2
  }
