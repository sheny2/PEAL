...
sigma_v_hosp <- runif(H, min = 1, max = 3)  # Varying patient-level variance by hospital
...
for(i in 1:H){
  ...
  df <- rearranged_data %>% filter(site == i) %>% dplyr::select(-c(site,Y, total_visits))
  ...
  covariate_cols <- colnames(df)[3:(ncol(df))]  # Select only X columns
...
points(sqrt(fit03.dlmm$V)[2:(1+px)], true_sigma[2:(1+px)], col = "blue")
...
# model <- lmer(Y ~ -1 + X1 + X2 + X3 + X4 + X5 + 
#               (1 | site) + (site + X1 + X2 + X3 + X4 + X5| site: patient), 
...
lmer.RS <- lmer(
  Y ~ -1 + X1 + X2 + X3 + X4 + X5 + 
       ...
       (0 + dummy(site, "4")| site:patient)  + (0 +  covar_composite_4 | site:patient) +  
       (0 + dummy(site, "5")| site:patient)  + (0 +  covar_composite_5 | site:patient),   
  data = three_lvl_dat %>%
    mutate(
      covar_composite_1 = dummy(site, "1") * 
        (X1 + X2 + X3 + X4 + X5),
      covar_composite_2 = dummy(site, "2") * 
        (X1 + X2 + X3 + X4 + X5),  
      covar_composite_3 = dummy(site, "3") * 
        (X1 + X2 + X3 + X4 + X5),
      covar_composite_4 = dummy(site, "4") * 
        (X1 + X2 + X3 + X4 + X5),  
      covar_composite_5 = dummy(site, "5") * 
        (X1 + X2 + X3 + X4 + X5)
    ),
  control = lmerControl(
    optimizer = "optimx",
...
fixed_effects <- fixef(lmer.RS)
plot(fixed_effects, true_beta)
abline(a = 0, b = 1, col = "blue", lwd = 2)
