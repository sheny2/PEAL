...
# Parameters
H <- 3 # of sites
m_hosp <- sample(250:300, H, replace = T) # of patients (1k to 3k later)
...
three_lvl_dat %>% 
  ...
  ggplot(aes(visit_time, fill = site)) + geom_histogram(bins = 50) + facet_wrap(~site) + 
  labs(x = "Visit Time") + theme(legend.position = "none")

three_lvl_dat %>% 
  mutate(site = factor(site)) %>% 
  ggplot(aes(visit_time, col = 5, fill = site, group = site)) + geom_histogram(bins = 50)  + 
  labs(x = "Visit Time") + theme(legend.position = "none")
