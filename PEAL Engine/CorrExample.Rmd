...
# Parameters
H <- 3  # Number of sites
m_hosp <- sample(50:75, H, replace = T) # Number of patients per site
...
n_visits <- sample(10:15, sum(nn), replace = TRUE)  # Number of visits of patients
...
## Correlated random effects at the patient level: RI and RS covariance correlated 

```{r}
source("../PEAL Engine/PEAL_Engine_CorrRS.R")
fit.peal_Corr = peal.fit.CorrRS(Y = NULL, X = NULL, Z = NULL,
                      id.site = NULL, weights = NULL,
                      pooled = F, reml = T,
                      common.s2 = T,
                      ShXYZ = ShXYZ,  # only need summary stats
                      ...
                      mypar.init = NULL)
c(fit.peal_Corr$b)
sqrt(fit.peal_Corr$V)[1:(1+H+H)]
sqrt(fit.peal_Corr$s2)
fit.peal_Corr$rho
```
...
fit3.lmer <- lmer(
  Y ~ -1 + X1 + X2 + X3 + X4 + X5 + X6 +
       (1 | site) +                          
       (0 + dummy(site, "1") + dummy(site, "2") + dummy(site, "3") +
          covar_composite_1 + covar_composite_2 + covar_composite_3 | site:patient),
  data = rearranged_data %>%
    mutate(
      covar_composite_1 = dummy(site, "1") * X6,
      covar_composite_2 = dummy(site, "2") * X6,
...
## more correlation between site and patient level random effects 

```{r}
source("../PEAL Engine/PEAL_Engine_CorrRS2.R")
fit.peal_Corr = peal.fit.CorrRS(Y = NULL, X = NULL, Z = NULL,
                      id.site = NULL, weights = NULL,
                      pooled = F, reml = T,
                      common.s2 = T,
