...
#     N <- N + Nh
#     pzh <- ncol(ShZ)
#     par_h <- c(par[1],                # site variance
#                par[1 + h],            # patient intercept variance
#                par[1 + K + h],        # patient slope variance
#                par[1 + 2*K + h])      # correlation
#
#     # cat(pzh, "\n")
#     if(pzh != length(c(par_h[1], rep(par_h[2], mh), rep(par_h[3], pzh - 1 - mh)))){cat("V dim problem")}
#
#
#     sigma_int   <- sqrt(par_h[2])
#     sigma_slope <- sqrt(par_h[3])
#     rho         <- par_h[4]
#
#     # patient-level 2x2 covariance
#     Sigma_patient <- matrix(c(sigma_int^2,
#                               rho * sigma_int * sigma_slope,
#                               rho * sigma_int * sigma_slope,
#                               sigma_slope^2), 2, 2)
#
#     # Repeat for each patient (mh of them)
#     block_patients <- Matrix::bdiag(replicate(mh$`n_distinct(patient)`, Sigma_patient, simplify = FALSE))
#
#     # Now build full V
#     V <- Matrix::bdiag(
#       par_h[1],        # site intercept variance
#       block_patients,  # correlated patient intercept + slope
#       diag(par_h[3], pzh - 1 - mh$`n_distinct(patient)`)  # remaining slopes if >1
#     )
#     V <- as.matrix(V)
#     Vinv <- solve(V)
#
...
#     px <- ncol(ShXYZ[[1]]$ShX)
#     K <- length(ShXYZ)
#     pz <- 1 + 3*K   # site, intercept, slope, corr
#   }
#
#   if (common.s2) {
#     ns <- 1
...
#       mypar.init <- c(1, rep(1, 2*K), rep(0, K))
...
#                  hessian = T,
#                  method = "L-BFGS-B",
#                  lower = c(rep(1e-6, 1 + 2*K), rep(-0.99, K)),
#                  upper = c(rep(Inf, 1 + 2*K),  rep(0.99, K)) )
...
lmm.profile3 <- function(par, pooled = FALSE, reml = TRUE,
  ...
  for (h in seq_along(id.site.uniq)) {
    ...
    rho           <- par[1 + 2*K + h] # correlation

    ## build covariance V
    sigma_int   <- sqrt(sigma_int2)
    sigma_slope <- sqrt(sigma_slope2)
...
peal.fit.CorrRS <- function(Y = NULL, X = NULL, Z = NULL, id.site = NULL, weights = NULL,
  ...
  ## init
  if (is.null(mypar.init)) {
    mypar.init <- c(1, rep(1, 2*K), rep(0, K))  # variances=1, rhos=0
